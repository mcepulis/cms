...
case 'published':
  ...
  comfirm($update_to_publish_status);
  break;
    ...
    case 'delete':
      ...
      case 'clone':
        ...
        $query = "INSERT INTO posts(post_category_id, post_title, post_author, post_user, post_date,post_image,post_content,post_tags,post_status) ";
          ...
          $query .= 
          "VALUES({$post_category_id},'{$post_title}','{$post_author}','{$post_user}',now(),'{$post_image}',
...
<?php
  ...
  $querySelect_posts = mysqli_query($connection, $query);
  while($row = mysqli_fetch_assoc($querySelect_posts)) {
    $post_id = $row['post_id'];
    $post_author = $row['post_author'];
    $post_user = $row['post_user'];
    $post_title = $row['post_title'];
